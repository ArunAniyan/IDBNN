    ...
    def __init__(self, in_channels: int, num_classes: int, num_boosters: int = 3):
        ...
        for _ in range(num_boosters):
            booster = nn.Sequential(
                nn.Linear(
                    in_channels + num_classes, 512
                ),  # Takes both features and previous predictions
                nn.ReLU(inplace=True),
                nn.Dropout(0.5),
...
class DBNNWithROI(nn.Module):
    ...
    def __init__(self, feature_extractor, dbnn, roi_size: int = 7):
        ...
        self.spatial_scale = (
            1.0 / 16.0
        )  # Adjust based on your feature extractor's downsampling
        self.dbnn = dbnn

    def forward(
        self, images: List[torch.Tensor], boxes: List[torch.Tensor]
    ) -> torch.Tensor:
        """
        Args:
        ...
        if not rois:  # No proposals
            return torch.zeros((0, self.dbnn.num_classes), device=features.device)
